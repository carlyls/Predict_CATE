...
# 
# #add for het intercept setting (1, 0.25, 0.25)
# target_sample_hetintercept <- target_sample %>%
#   mutate(eps_m = rnorm(n=1, mean=0, sd=1),
#          eps_tau = rnorm(n=1, mean=0, sd=0.25),
#          eps_age = rnorm(n=1, mean=0, sd=0.25))
# saveRDS(target_sample_hetintercept, here("Data", "target_sample_hetintercept.RDS"))
# 
# #add for het main setting (1, 0.5, 0.25)
# target_sample_hetmain <- target_sample %>%
#   mutate(eps_m = rnorm(n=1, mean=0, sd=1),
#          eps_tau = rnorm(n=1, mean=0, sd=0.5),
#          eps_age = rnorm(n=1, mean=0, sd=0.25))
# saveRDS(target_sample_hetmain, here("Data", "target_sample_hetmain.RDS"))
# 
# #add for het interaction setting (1, 1, 0.5)
# target_sample_hetinteraction <- target_sample %>%
#   mutate(eps_m = rnorm(n=1, mean=0, sd=1),
#          eps_tau = rnorm(n=1, mean=0, sd=1),
#          eps_age = rnorm(n=1, mean=0, sd=0.5))
# saveRDS(target_sample_hetinteraction, here("Data", "target_sample_hetinteraction.RDS"))
...
sample_dist <- function(k, n, Sigma, eps_study_m, eps_study_tau, 
  ...
  if (distribution == "same") {
    mu <- c(age=0, sex=0.6784, smstat=0.3043, weight=0, madrs=0)
      ...
      mu <- c(age = rnorm(1, 0, 0.2), sex = 0.6784 + rnorm(1, 0, 0.1), 
              smstat = 0.3043 + rnorm(1, 0, 0.1), weight = 0 + rnorm(1, 0, 0.5),
              madrs = 0 + rnorm(1, 0, 0.3))
