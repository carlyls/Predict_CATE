...
```{r, eval=F}
#read in all files
files <- list.files(path=here("Data","NonNormal"), pattern=".Rdata")

all_res <- target_res <- c()
setwd("~/Library/CloudStorage/Box-Box/Home Folder cb644/Documents/Research/Carly Projects/Moderation/Predict_CATE/Data/NonNormal")
...
nrow(target_res)/800
saveRDS(all_res, here("Data", "all_res_nonnormal_Jul2025.RDS"))
saveRDS(target_res, here("Data", "target_res_nonnormal_Jul2025.RDS"))
```

```{r, message=F, warning=F}
all_res <- readRDS(here("Data", "all_res_nonnormal_Jul2025.RDS"))
target_res <- readRDS(here("Data", "target_res_nonnormal_Jul2025.RDS"))
...
target_ind %>%
  ...
  facet_grid(sds_lab ~ Linear, scales = "free") +
...
ggsave(here("Results", "NonNormal", "coverage_nonnormal_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "coverage_nonnormal_linear_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "coverage_age_nonnormal_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "length_nonnormal_Jul2025.jpeg"))
```
...
target_ind %>%
  ...
  geom_smooth(se=F) +
  facet_grid(sds_lab ~ Linear, scales = "free") + theme(text = element_text(size=15)) +
  xlab("Standardized Age") +
  ylab("Interval Length in Target Sample") +
  labs(color = "Method")

ggsave(here("Results", "NonNormal", "length_age_nonnormal_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "absbias_nonnormal_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "bias_nonnormal_Jul2025.jpeg"))
```
...
ggsave(here("Results", "NonNormal", "absbias_age_nonnormal_Jul2025.jpeg"))
...
ggsave(here("Results", "NonNormal", "bias_age_nonnormal_Jul2025.jpeg"))
```
